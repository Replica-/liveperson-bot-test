...
class AgentChat {
    ...
    /*
    *   Requests External IP (Server ip) from http://jsontest.com
    *   Returns JSON format of IP
    */
    _getExternalIP(callback) {

        const options = {
            method: 'GET',
            url: `${config.endpoints.requestExternalIp}`,
            json: true
        };
        ...
        request(options, (error, response, body) => {
            ...
            if (error) {
                callback(`Failed to retrieve IP address using jsontest.com with error: ${JSON.stringify(error)}`);
            }
            else if(response.statusCode < 200 || response.statusCode > 299){
                callback(`Failed to retrieve IP address with error: ${JSON.stringify(body)}`);
            }
            console.log(`Retreived IP - ${body.ip}`);
            callback(null, {
                ip: body.ip
            });
        });
            ...
            if (events) {
                ...
                for (let i = 0; i < events.length; i++) {
                    ...
                    if ((ev['@type'] === 'state') && (ev.state === 'ended')) {
                        ...
                        console.log(`(chatPolling) - line from visitor:${ev.text}`);
                        this._processVisitorLine(ev);
                    }
                }
            }
    ...
    /*
    *   Process Visitor Line
    *   Based on hardcoded values it will return an appropiate response
    */
    _processVisitorLine(event) {
        if (event.text) {
            if (event.text.toLowerCase() == "what is the server ip?") {

                // Keep it consistent with async
                async.series([
                    callback => {
                    this._getExternalIP(callback);
                }], (err, data) => {
                    if (err) {
                        console.log('Error:' + err);
                        this.sendLine("I am unable to find the server's IP at the moment!");
                    }
                    else {
                        console.log('AgentChat Retrieved IP');
                        this.sendLine(data[0].ip);
                    }
                });
            }
        }
